...
- `infra`: Docker Compose, Traefik, deployment/runtime wiring only.
  ...
  - `docker-compose.prod.yml` — **standalone** production deployment; pulls pre-built images from GHCR (`ghcr.io/<owner>/...`), runs the `migrate` service on startup, uses production domains (`mcp.alexiuc.dev`, `hub.alexiuc.dev`). Connects to the external `proxy` network. Usage: `docker compose --project-name my-hub-prod --env-file .env -f infra/docker-compose.prod.yml up -d`.

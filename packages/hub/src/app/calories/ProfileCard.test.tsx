...
describe('ProfileCard', () => {
  ...
  it('accepts stored decimal values that a fixed step would reject', () => {
    ...
    const rate = screen.getByLabelText(/rate \(kg\/week\)/i) as HTMLInputElement;
    ...
    for (const label of [/height/i, /^weight/i, /goal weight/i, /gym day bonus|bonus/i]) {
      const field = screen.queryByLabelText(label) as HTMLInputElement | null;
  ...
  it('pre-fills the goal weight from the profile', () => {
    ...
    expect((screen.getByLabelText(/goal weight/i) as HTMLInputElement).value).toBe('87.9');
  });
  ...
  it('offers no goal weight for a maintain goal, which has no finish line', () => {
    ...
    expect(screen.queryByLabelText(/goal weight/i)).toBeNull();
  });
});

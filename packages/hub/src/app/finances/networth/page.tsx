...
export default function NetWorthPage() {
  ...
  return (
    <div className="flex flex-col gap-[14px]">
      ...
      >
        <div className="mb-1.5 text-xs" style={{ color: 'var(--fin-muted)' }}>
          Current Net Worth
        </div>
        <div className="mb-1.5 text-[34px] font-bold tracking-[-0.02em]" style={{ color: 'var(--fin-text)' }}>
          {fmt(netWorth, currency)}
        </div>
        {deltaVsLastMonth != null && (
          <div
            className="mb-4 text-xs"
            style={{ color: deltaVsLastMonth >= 0 ? 'var(--fin-green)' : 'var(--fin-red)' }}
          >
        ...
        {history.length >= 2 ? (
          <>
            ...
            <div className="mt-1 flex justify-between">
              {history.map(s => (
                <span key={s.month} className="text-[9px]" style={{ color: 'var(--fin-subtle)' }}>
                  {s.label}
                </span>
              ))}
            </div>
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        ...
        <Card className="px-[14px] py-3">
          <div className="mb-1 text-[10px] uppercase tracking-[0.08em]" style={{ color: 'var(--fin-red)' }}>
            Liabilities
          </div>
          <div className="text-xl font-bold" style={{ color: 'var(--fin-red)' }}>
                ...
                <div key={a.id}>
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="text-[13px]" style={{ color: 'var(--fin-text)' }}>
                        {a.name}
                      </div>
                    ...
                    <div className="text-sm font-semibold" style={{ color: 'var(--fin-text)' }}>
              ...
              return (
                <div key={a.id}>
                  ...
                  >
                    ...
                    <div className="text-sm font-semibold" style={{ color: 'var(--fin-red)' }}>
                      -{fmt(a.balance, a.currency)}
                    </div>
                  </div>
                </div>
        ...
        <Card className="p-[14px]">
          ...
          <div className="grid grid-cols-4 gap-0">
            {['Month', 'Assets', 'Liab.', 'Net'].map(h => (
              <div
                key={h}
                className="px-1 pb-2 text-[9px] uppercase tracking-[0.06em]"
                style={{
            ...
            {history.map((row, i) => {
              ...
              const cells = [
                <span style={{ color: 'var(--fin-muted)' }}>{row.label}</span>,
                <span style={{ color: 'var(--fin-green)', fontVariantNumeric: 'tabular-nums' }}>
                  {fmt(row.totalAssets, currency)}
                </span>,
                <span style={{ color: 'var(--fin-red)', fontVariantNumeric: 'tabular-nums' }}>
                  -{fmt(row.totalLiabilities, currency)}
                </span>,
                <span style={{ fontWeight: 600, color: 'var(--fin-text)', fontVariantNumeric: 'tabular-nums' }}>
              ...
              return cells.map((cell, ci) => (
                <div
                  ...
                  style={{
                    ...
                    background: isLatest ? 'var(--fin-accent-d)' : 'transparent',
                  }}
                >
                  {cell}
                </div>

...
export default function TravelPage() {
  ...
  const loadTrips = useCallback(async () => {
    ...
    try {
      ...
      const data = (await res.json()) as { trips: Trip[]; booking_ranges?: BookingRange[] };
      setTrips(data.trips);
  ...
  async function addChecklistItem() {
    if (!activeTripId || !newChecklistItem.trim()) return;
  ...
  async function addCompanion() {
    if (!activeTripId || !newCompanionName.trim()) return;
  ...
  async function addBooking() {
    if (!activeTripId || !newBookingTitle.trim()) return;
  ...
  async function saveBookingEdits(bookingId: number) {
    if (!activeTripId || !editBookingTitle.trim()) return;
  ...
  async function saveChecklistEdits(itemId: number) {
    if (!activeTripId || !editChecklistTitle.trim()) return;
  ...
  async function uploadDocument() {
    if (!activeTripId || !documentFile) return;
                      ...
                      {editingChecklistId === item.id ? (
                        ...
                        <>
                          <button
                            ...
                            className={`min-w-0 flex-1 text-left ${item.done ? 'line-through' : ''}`}
                          >
                            {item.title}
                          </button>
                          <div className="flex items-center gap-1">
            ...
            <SectionCard title="Documents" className="bg-amber-950/20 border-amber-800/50">
              <div className="space-y-2 mb-3">
                ...
                <button
                  ...
                  disabled={!activeTripId || !documentFile}

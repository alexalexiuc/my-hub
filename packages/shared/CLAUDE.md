...
| `monthly-plans.ts` | Monthly plan CRUD + auto-match: getOrCreateMonthlyPlan, getMonthlyPlanFull (plan+items+summary), updateMonthlyPlanAvailableAmount (availableAmount and/or incomeAccountId), addPlanItem, updatePlanItem, deletePlanItem, togglePlanItemAssigned, bulkAssignAll, copyToNextMonth, tryAutoMatchPlanItems (matches plan items and auto-adds income into monthly available funds when transaction account matches plan incomeAccountId); Types: PlanItemInsert, PlanItemUpdate, MonthlyPlanFull, MonthlyPlanSummary, PlanItemWithMeta |

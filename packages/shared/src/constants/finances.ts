...
// ---- Transaction extras (financeTransactions.extras) ----
// Structured metadata optionally added by the MCP server / AI at insert time.
// All fields are optional — only populated when relevant.

export interface TransactionExtras {
  // Source of the transaction entry: 'mcp' | 'hub' | 'import'
  source?: string;
  // Confidence score (0–1) when category/merchant was auto-suggested
  autofillConfidence?: number;
  // Raw input the AI parsed this transaction from (e.g. the user's message)
  rawInput?: string;
  // Card hint used to match to an account (e.g. "Visa *4242")
  cardHint?: string;
  // Any additional structured context the AI wants to persist
  [key: string]: unknown;
}

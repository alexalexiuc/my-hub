...
// Helper: open the inline add input and type a title, then press Enter to save
...
test.describe('Todo', () => {
  ...
  test('page shows Todo heading and add button', async ({ page }) => {
    await expect(page.getByRole('heading', { name: 'Todo', level: 1 })).toBeVisible();
    // The add button is an icon button with title="Add task"
  ...
  test('marks a todo as done', async ({ page }) => {
    await addTodo(page, 'Finish this task');
    ...
    // The item should now be in the Done section
  ...
  test('done item no longer appears in Open section', async ({ page }) => {
    ...
    // Open count goes back to 0 — empty state message appears

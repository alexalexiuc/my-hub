...
import { requireHubAuthExtra, toolResponse } from '../../shared/toolsUtils';
...
export const addTodoTool: ToolHandler<typeof AddTodoSchema.shape> = async (input, extra) => {
  const { userId } = requireHubAuthExtra(extra);
  const todo = await addTodo(userId, input.title);
  return toolResponse(todo);
};

export const markDoneTool: ToolHandler<typeof MarkDoneSchema.shape> = async (input, extra) => {
  const { userId } = requireHubAuthExtra(extra);
...
export const listTodosTool: ToolHandler<typeof ListTodosSchema.shape> = async (_input, extra) => {
  const { userId } = requireHubAuthExtra(extra);

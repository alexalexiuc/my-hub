...
| ID    | Requirement                                                                                                                                                                                                                                                                                      |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| FR-10 | A weekly report email is sent every Monday at 08:00 (UTC), covering the previous Mon–Sun week.                                                                                                                                                                                                   |
| FR-11 | A monthly report email is sent on the 1st of each month at 08:00 (UTC), covering the prior calendar month.                                                                                                                                                                                       |
...
| FR-16 | A POST `/api/calories/reports/weekly-trigger` endpoint allows the authenticated user to manually trigger their own weekly report email (for testing).                                                                                                                                            |
| FR-17 | A POST `/api/calories/reports/monthly-trigger` endpoint allows the authenticated user to manually trigger their own monthly report email (for testing).                                                                                                                                          |
| FR-18 | Manual trigger endpoints return `{ sent: true }` if the email was sent, or `{ skipped: "no_data" }` if no meals were logged in the period.                                                                                                                                                       |

### Technical Requirements

| ID    | Requirement                                                                                                                                                                                                                          |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| TR-10 | Email HTML is built by TypeScript template functions (no template engine): `buildWeeklyReportHtml(data: WeeklyReportData)` and `buildMonthlyReportHtml(data: MonthlyReportData)` in `packages/shared/src/services/email/templates/`. |

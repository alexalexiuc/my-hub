...
| TR-16 | Monthly report verdict is tiered: ≥60% days on target = "On track" (green), 40–59% = "Close" (amber), <40% = "Over" (red). This replaces the previous binary 50% threshold to provide more nuance.                                                                                                                                                                                                                                                                                                                                                            |
| TR-17 | Monthly report week-by-week bar chart scale is dynamic: derived as `max(goal × 1.3, maxWeeklyKcal × 1.1)` to ensure the goal line remains proportionally clear and bars never exceed 100% regardless of user goal or logged intake variance.                                                                                                                                                                                                                                                                                                                  |
| TR-18 | Monthly report macros section is guarded: if <3 days logged, show a note "Log at least 3 days to see meaningful macro trends" instead of rendering unreliable percentages.                                                                                                                                                                                                                                                                                                                                                                                    |
| TR-19 | Monthly report body composition section gracefully handles missing measurements: if no start or end measurements exist at all, show a helpful prompt "Start logging body measurements…" instead of an empty table.                                                                                                                                                                                                                                                                                                                                            |

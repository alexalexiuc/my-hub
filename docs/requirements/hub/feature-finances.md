...
| TR-21 | Payees must support optional alias names so AI/imported transactions can resolve legal or statement names to an existing canonical payee, and both payees and categories must support optional notes metadata without changing the transaction-entry UI. |
...
- [x] Payees can store aliases and notes, alias-based transaction creation resolves to the canonical payee, and categories can store optional notes without adding extra fields to the transaction modal.

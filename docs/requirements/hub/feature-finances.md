...
| TR-37 | `getSavingsAndDebtFlows(userId, budgetId, dateFrom, dateTo)` in shared `reporting.ts` sums transfer transactions (in the budget's default currency) by destination account type for a date range: `savings` (Goal + Tracking accounts), `investments` (Investment accounts), and `debtRepayment` (Loan accounts). Credit Card repayments are excluded — they are already reflected as spending via `getAccountsCashflow`.                                                     |
| TR-38 | The Reporting page API exposes `savingsDebtFlows` (current + previous period amounts for savings, investments, debt repayment) and `groupBreakdown` (categorized expense/income totals for the active period rolled up by category group, with `prevAmount`/`pct`, "Ungrouped" for categories without a group). The Reporting page renders these as a "Savings, Investments & Debt" card and a "By Category Group" card, side by side below the By Category / Top Payees row. |

...
| FR-09 | The main dashboard must display a compact Travel widget when an ongoing or upcoming trip exists, with minimal details and a link to `/travel`. |
...
| FR-19 | The Calories Progress tab must display a goal progress card in kg, using goal type (lose, maintain, gain). Its projection is anchored to the profile's stored goal baseline (`goalStartDate`/`goalStartWeightKg`), **not** to the current week's first weigh-in: a cumulative line, so a week that ends over target carries the miss into the next week's target instead of rebasing onto it. The card is judged on the smoothed trend weight rather than the raw reading, offers 4W/8W/12W/All windows (4W default), and exposes a "Restart from today" action as the only way the baseline moves after the goal is set. See `docs/requirements/mcps/feature-calorie-tracker.md` FR-42/FR-43/FR-44. |
...
- [x] Goal progress shows trend vs projected weight in kg and adapts projection direction to goal type.
- [x] A week that ends over target does not become the next week's baseline — two weeks of a 1 kg/week goal target 2 kg down, whatever the scale did in between.
- [x] A one-day water-weight spike moves the goal verdict by a fraction of what it moves the scale.
- [x] Goal progress and the weight trend chart each offer a selectable window.

...
| TR-02 | Decimal/float columns use Drizzle `real()` by default (PostgreSQL `REAL`) to avoid string↔number round-trips. Exception: finance money/exchange-rate fields use `numeric(18,4                                                       | 8)`through`numericCasted`in`packages/shared/src/db/schema/finances.ts`for fixed-scale precision while remaining JS`number` in app code. |

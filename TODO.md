...
- [ ] Add a column to DB tables that store non-config data (calories, travel, finances, profile, etc.) indicating how the data was added (MCP, hub, or other), to make it easier to identify the data source and optimize queries
...
- [ ] Finances (budget): reduce cross-process cache staleness between Hub and MCP (`budgetAccessCache`, `budgetsCache`), and evaluate a PromiseCacheX wrapper with table-based invalidation hooks (optionally PG NOTIFY-backed) plus deterministic cache reset for MCP E2E setup.
